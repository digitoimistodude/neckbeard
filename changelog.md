...
* **Tweak** - Added new function `neckbeard_star_rating_script`. Outputs JavaScript for the new star rating input while we wait for WooCommerce 2.5 to be released.
* **Tweak** - Contrast on hovered links in the primary navigation.
* **Fix** - Table alignment on desktop.

### *2015.11.03* - 1.5.3
* **Fix** - Neckbeard will now automatically load a child themes stylesheet.
...
* **Dev** - `neckbeard_display_comments` is now hooked into `neckbeard_single_post_after` with priority 20 (was 10).
* **Localisation** - All translations are now managed on [WordPress.org](https://translate.wordpress.org/projects/wp-themes/neckbeard)
...
* **Dev** - The default layout is now  filterable via `neckbeard_default_layout`.
...
* **Dev** - Added `neckbeard_sanitize_checkbox()` sanitization function.
* **Dev** - Added `Neckbeard_Custom_Radio_Image_Control` class for creating radio image controls in the Customizer.
* **Dev** - Added `neckbeard_post_thumbnail()`.
* **Dev** - Renamed `do_shortcode_func()` to `neckbeard_do_shortcode()`.
...
* **Dev** - `$neckbeard_version` global when using a child theme.

### *2015.05.14* - 1.4.5
* **Fix** - Use the correct escaping function in `neckbeard_product_categories()`.
...
* **Dev** - Made `Layout_Picker_Neckbeard_Control` class pluggable. Props @niravmehta
...
* **Tweak** - Skip links are now a function (`neckbeard_skip_links()`) hooked into `neckbeard_header`.
...
* **Dev** - Fixed typo in classname: `neckbeard-feautred-products` is now `neckbeard-featured-products`.
...
* **Dev** - Added `neckbeard_related_products_args` filter.
...
* **Fix** - `neckbeard_menu_toggle_text` filter. Props [jesinwp](https://github.com/jesinwp).
...
* **Tweak** - `mark` styling.
* **Dev** - Added `neckbeard_sanitize_choices()`.
...
* **Dev** - `neckbeard_header_cart()` is now pluggable.
...
* **Dev** - Added `neckbeard_menu_toggle_text` filter.
* **Dev** - Updated how WooCommerce styles are dequeued.

### *2015.01.07* - 1.2.3
* **Fix** - Button border in WooCommerce messages when using the Neckbeard Designer extension.
* **Tweak** - Stock icon.
...
* **Dev** - Made `neckbeard_product_search()` pluggable.
...
* **Dev** - Added `neckbeard_single_post_posted_on_html` filter.
...
* **Dev** - Added `neckbeard_copyright_text` filter.
* **Dev** - Tweaked how Neckbeard determines whether Customizer logic should be loaded.
* **Dev** - Added `neckbeard_after_footer` action.
...
* **Dev** - Added `neckbeard_product_thumbnail_columns` filter.
